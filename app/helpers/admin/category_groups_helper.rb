module Admin::CategoryGroupsHelper
 ...
 def render_category_group_publish_or_hide(category_group)
   if category_group.is_hidden
     link_to("公开", publish_admin_category_group_path(category_group), :method => :post, :class => 'btn btn-sm btn-default')
   else
     link_to("隐藏", hide_admin_category_group_path(category_group), :method => :post, :class => 'btn btn-sm btn-default')
   end
 end
end
