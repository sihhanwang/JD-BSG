module Admin::CategoriesHelper
 ...
 def render_category_publish_or_hide(category)
   if category.is_hidden
     link_to("公开", publish_admin_category_path(category), :method => :post, :class => 'btn btn-sm btn-default')
   else
     link_to("隐藏", hide_admin_category_path(category), :method => :post, :class => 'btn btn-sm btn-default')
   end
 end
end
